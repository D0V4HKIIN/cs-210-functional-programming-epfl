...
trait Solver extends GameDef:
  ...
  def from(
    ...
    if initial.isEmpty then LazyList()
      ...
      lazy val nextNeighbours = initial.flatMap(pos =>
        newNeighborsOnly(neighborsWithHistory(pos._1, pos._2), explored)
      )

      nextNeighbours ++ from(
        nextNeighbours,
        explored ++ nextNeighbours.map(b => b._1)
      )
  ...
  lazy val solution: List[Move] =
    if pathsToGoal.isEmpty then Nil else pathsToGoal.head._2.reverse
