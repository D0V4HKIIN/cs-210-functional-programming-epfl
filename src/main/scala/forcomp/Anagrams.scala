...
object Anagrams extends AnagramsInterface:
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary
      .groupBy((word: Word) => wordOccurrences(word))
    ...
    occurrences match
      ...
      case x :: xs =>
        (for
          ...
          amount <- (0 to x._2) // could 1 to x._2
        yield
          if amount == 0 then combination
          else
            (x._1, amount) :: combination
        ) // with ::: combinations(xs), thus removing the need for the if statement
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val empt: Occurrences = Nil
    x.foldLeft(empt)((acc, elem) =>
      val amount = elem._2 - (
        y.toMap.get(elem._1) match
          case None        => 0
          case Some(value) => value
      )
      if amount != 0 then
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def occurenceAnagrams(occ: Occurrences): List[Sentence] =
      occ match
        case Nil => List(List())
        case _ =>
          for
            ...
            rest <- occurenceAnagrams(subtract(occ, combination))
          yield anagram :: rest
