...
trait Huffman extends HuffmanInterface:
  ...
  def weight(tree: CodeTree): Int =
    tree match
      case Leaf(char, w)               => return w
      case Fork(left, right, chars, w) => return w

  def chars(tree: CodeTree): List[Char] =
    tree match
      case Leaf(char, weight) => List(char)
      case Fork(left, right, char, weight) =>
        (chars(left) ::: chars(right)).distinct
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    def getList(l: List[(Char, Int)]): List[Leaf] =
      def max(l: List[(Char, Int)]): Leaf =
        if l.tail.isEmpty then Leaf(l.head._1, l.head._2)
        else
          val maxTail = max(l.tail)
          if l.head._2 > maxTail.weight then Leaf(l.head._1, l.head._2)
          else Leaf(maxTail._1, maxTail._2)
      if l.isEmpty then List.empty
      else
        val maximum = max(l)
        getList(l.filterNot(_._1 == maximum.char)).concat(List(maximum))
    getList(freqs);
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    trees match
      ...
      //       trees.head,
      //       trees.tail.head,
      //       (chars(trees.head) ::: (chars(trees.tail.head))).distinct,
  ...
  def until(
    ...
    def work(trees: List[CodeTree]): List[CodeTree] =
      if done(trees) then trees
      else work(merge(trees))
    work(trees)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    def getChar(sub_tree: CodeTree, bits: List[Bit]): (Char, List[Bit]) =
      sub_tree match
        case Leaf(char, _) => (char, bits)
        case Fork(left, right, _, _) =>
          bits match
            case 0 :: xs => getChar(left, xs)
            case 1 :: xs => getChar(right, xs)

    val (char, new_bits) = getChar(tree, bits)
    new_bits.length match
      case 0 => List(char)
      case _ => char :: decode(tree, new_bits)

    // can probably use decode instead of this
    // def getChars(sub_bits: List[Bit]): List[Char] =
    //   val (char, new_bits) = getChar(tree, sub_bits)
    //   new_bits.length match
    //     case 0 => List(char)
    //     case _ => char :: getChars(new_bits)
    // getChars(bits)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def getEncodedChar(
      ...
      subTree match
        case Leaf(leafChar, _) => bits
        case Fork(left, right, chars, _) =>
          left match
            case Fork(_, _, chars, _) =>
              if chars.contains(char) then getEncodedChar(left, char, bits :+ 0)
  ...
  def convert(tree: CodeTree): CodeTable =
    def convertSubTree(tree: CodeTree, acc: List[Bit]): CodeTable =
      tree match
        case Leaf(char, weight) => List((char, acc))
        case Fork(left, right, chars, weight) =>
          mergeCodeTables(
            convertSubTree(left, acc :+ 0),
            convertSubTree(right, acc :+ 1)
          )
