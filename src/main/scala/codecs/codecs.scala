...
trait DecoderInstances:
  ...
  given [A](using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(items) => Some(items.foldRight(List[A]())((j, acc) =>  j.decodeAs.get :: acc))
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction{case Json.Obj(obj) if obj.contains(name) && decoder.decode(obj(name)).isDefined => decoder.decode(obj(name)).get}
