...
class ListsSuite extends munit.FunSuite:
    ...
    /** Tests are written using the `test("description") { ... }` syntax The
      * most common way to implement a test body is using the method `assert`
      * which tests that its argument evaluates to `true`. So one of the
      * simplest successful tests is the following:
      */
    ...
    /** One problem with the previous (failing) test is that munit will only
      * tell you that a test failed, but it will not tell you what was the
      * reason for the failure. The output looks like this:
      *
      * {{{
      ...
      * }}}
      *
      * We recommend to always use the assertEquals equality operator when
      * writing tests.
      */
